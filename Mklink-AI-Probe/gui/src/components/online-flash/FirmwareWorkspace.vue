...
<template>
  ...
  <div class="firmware-toolbar">
    ...
    <label v-if="trackedBrowserPicker && !nativeApp" class="file-button" role="button" tabindex="0" :title="tr('上传当前文件副本，不跟随源文件更新', 'Upload a snapshot without tracking source changes')" @keydown.enter.prevent="openFile" @keydown.space.prevent="openFile">{{ tr('上传副本', 'Upload copy') }}<input ref="fileInput" class="visually-hidden" data-testid="firmware-copy-input" type="file" accept=".bin,.hex" @change="fileChanged"></label>
    <span class="filename" :title="sourceName">{{ sourceName || tr('拖拽 BIN / HEX 到此处，或选择固件', 'Drop BIN / HEX here, or select firmware') }}</span>
  ...
  <div class="hex-scroll" @scroll="scrolled">
    ...
    <div v-if="!inspection && !memoryData" class="empty">{{ tr('选择固件即可解析预览；选定器件后检查是否可烧录。预览按需加载。', 'Select firmware to preview it, then choose a target to validate programming. Preview data loads on demand.') }}</div>
  </div>
  </div>
</template>

<style scoped>
.firmware-workspace{position:relative;min-height:0;height:100%;display:flex;flex-direction:column;outline:2px solid transparent;outline-offset:-2px}.firmware-workspace.dragging{outline-color:var(--of-accent);background:color-mix(in srgb,var(--of-accent) 8%,transparent)}.firmware-toolbar{display:flex;flex-wrap:wrap;align-items:center;gap:8px;padding:10px;border-bottom:1px solid var(--of-border)}.file-button{flex:0 0 auto;white-space:nowrap;line-height:1.4;padding:7px 10px;border:1px solid var(--of-border);border-radius:5px;background:var(--of-input);color:var(--of-text);font-size:11px;cursor:pointer}.visually-hidden{position:absolute!important;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.file-button:focus-visible{outline:2px solid var(--of-accent);outline-offset:2px}.filename{flex:1 1 140px;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:var(--of-muted);font-size:11px}.inspection-status{margin-left:auto;color:var(--of-warn);font-size:10px;white-space:nowrap}.inspection-ok{color:var(--of-ok)}.base-field{margin-left:auto;display:flex;flex:0 0 auto;align-items:center;gap:5px;color:var(--of-muted);font-size:10px;white-space:nowrap}.base-field+.inspection-status{margin-left:0}.base-field input{flex:0 0 92px;width:92px;min-width:92px;padding:6px;border:1px solid var(--of-border);border-radius:4px;background:var(--of-input);color:var(--of-text);font-family:var(--of-mono)}.file-actions{display:flex;flex-wrap:wrap;max-width:100%;flex:0 1 auto;gap:6px;margin-left:auto}.file-action{display:inline-flex;flex:0 0 auto;align-items:center;justify-content:center;gap:5px;min-height:30px;padding:6px 9px;border:1px solid var(--of-border);border-radius:5px;background:var(--of-input);color:var(--of-text);font-size:11px;white-space:nowrap}.file-action:disabled{cursor:not-allowed;opacity:.45}.error{margin:5px 10px;color:var(--of-danger);font-size:11px}.metadata{display:flex;flex-wrap:wrap;gap:14px;padding:7px 10px;border-bottom:1px solid var(--of-border);color:var(--of-muted);font-size:10px}.hex-head,.hex-row{display:grid;grid-template-columns:78px minmax(430px,1fr) 136px;align-items:center;white-space:pre}.hex-head{padding:6px 10px;background:var(--of-surface);color:var(--of-muted);font:10px var(--of-mono)}.hex-scroll{min-height:0;height:auto;flex:1;overflow:auto;text-align:left;background:var(--of-bg);font:11px/20px var(--of-mono)}.hex-row{height:20px;padding:0 10px;color:var(--of-text)}.cells{display:grid;grid-template-columns:repeat(16,2ch);column-gap:1ch}.cells i{font-style:normal;color:var(--of-text)}.cells i.gap{color:var(--of-muted)}.empty{padding:50px 20px;text-align:center;color:var(--of-muted)}
</style>
