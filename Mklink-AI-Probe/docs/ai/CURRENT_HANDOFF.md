...
- 更新时间：`2026-09-21T14:38:47+08:00`
- 分支：`codex/release-022-handoff`
- HEAD：`基于已发布 main 424f470；本分支只整理交接和发布记录。`
- 远端 HEAD：`PR #5 已合并；v0.2.2 固定在 424f4700a4d9726db1e9e1a7cfbadaee68a737e6。`
- 工作树：应用/固件发布已完成。原主工作区的用户固件替换、固件源码和 MicroBoot 修改独立保留。
- 当前任务：应用0.2.2与四份最新固件已正式发布；整理精简交接。临时目录删除被自动审批拒绝，未删除；后续需由维护者处理。
- 状态：`complete`
...
2. 交接分支提交PR后由维护者决定合并；从主工作区更新源码前核对用户固件替换，勿reset/stash未知修改。
3. 如需V2自动升级，单独实现RBL类型、校验与升级路径并回归；当前只提供手动RBL下载。
4. 临时目录删除被自动审批拒绝（仅返回blocked by policy）。尚未清理.build/runs；含符号链接目录须单独检查。保留发布包、历史备份、HIL及共享依赖缓存。
