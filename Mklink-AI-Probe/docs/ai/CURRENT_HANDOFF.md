...
- 更新时间：`2026-09-19T01:08:36.0320714+08:00`
...
- 当前任务：HPM5301第二颗完成实验根哈希、签名启动正例、SECURE→RETURN及Chrome读写；修复SECURE伪数据配置解析与连接切换旧OTP计划。等待新芯片继续NONRET等未验证分支。
- 状态：`in_progress`
...
- **V2 narrow reads and shared RTT validation**：0.2.2证据：v0.2.2-rtt-narrow-alignment.md（V2窄值/RTT及三代编译）；v0.2.2-superwatch-presentation.md（V2共享流891万样本）；v0.2.2-svd-selection.md（Chrome芯片切换）；v0.2.2-v3-regression.md（V3四档、UART/RTT/CLI/MCP/LA；在线30M两模式各10次修复后通过，脱机17次）；v0.2.2-gui-session-layout.md（176 Python/706 GUI，V4共享流734万样本、长路径拖动及SV/UART）；v0.2.2-v4-narrow-stream.md（三档静态8通道/动态6通道30秒通过，SansIO断流修复223项回归，清理超时后手动恢复）；v0.2.2-offline-installer.md（完整离线包304654762字节，7059/2224及SansIO入包通过；UAC取消，覆盖安装未验收）。报告均在docs/verification。未发布。 HPM用户OTP: v0.2.2-hpm5301-user-otp.md，24 Python/6 GUI/编译通过；word69两次烧位0→1→3，Chrome写入与影子回读一致、邻字不变、同值跳过、LDO关闭、LOCK及目标tick增长；修复复位后OTP访问临时4MHz并恢复档位。组19永久锁已实测：HARD_LOCK=0x30480016；word75=1/word79=0x80000003；目标ROM锁区写回读不变，UART连续。调试禁用后续结果见独立安全报告。 docs/verification/v0.2.2-hpm5301-security.md：SW_VER=1拒绝旧启动头、更新启动头后启动通过；TCU熔丝保持、DEBUG禁止SBA且保留ID、JTAG禁止ID且UART继续启动。修复SBA不可用误报全零和Device短读成功，52项Python通过。未锁目标回归、物理断电、生命周期、签名/密钥功能待测；不宣称全部安全验收。 2026-09-19第二颗：38,132B正常ROM下载/校验、256位根哈希、38,144B签名启动、SECURE→RETURN、RETURN未签名运行及Chrome word69/79写入通过；SECURE全地址固定20150407，由实时CHIP_ID校验拒绝配置解析；RETURN控制字写锁预检拒绝。38 Python、8 GUI及生产构建通过。10份SDK OTP表审计发现基址/EXIP长度差异，非全系列硬件资格。详见同报告。
...
- **state**：V4 HPMLink + HPM5301：当前芯片永久禁用DEBUG/JTAG，IDCODE=0；UART0仍持续运行。请保留用于物理断电复测，待用户确认换空白芯片再继续。
...
1. 第二颗HPM5301已为RETURN，不能回退SECURE；用户换新后继续NONRET、签名失败/撤销、密钥功能验证。保留两颗已测试芯片作物理断电复测。上位机修复仅开发分支，未经新授权不合并发布。
