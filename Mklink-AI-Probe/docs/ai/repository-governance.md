...
- [主线 PR 与 CI](https://github.com/MicroKeen/Mklink-AI-Probe/rules/22630535)：
  必须通过 PR；2026-09-15 按维护者最新授权永久取消第二人审批门槛：
  审批人数为 0，关闭最后推送审批和未归属修改的附加审批；
...
- [主线合并人员](https://github.com/MicroKeen/Mklink-AI-Probe/rules/22630539)：
  ...
  这不绕过前一条独立的 PR 与 CI 规则，因此不能直接推送主线。
...
### PR 无强制第二人审批（2026-09-15 最新授权）

维护者最新要求适用于全部 PR：不再要求另一人批准后才能合并。此规则取代此前仅对版本发布临时豁免的方案，后续发布不再临时修改或恢复审批门槛。不增加管理员绕过者。

合并前仍须检查差异、解决冲突、完成相关验证和必需 CI，使用确切头提交合并。AI 仍需用户明确授权合并；开发授权不自动包含合并或发布。必须走 PR、CI、讨论解决、禁止强推和删除的规则保留。
...
4. 核对改动与验证；CI 通过后由 Aladdin-Wang 或 su5176 合并，不需要另一人审批。
5. 发版由 Aladdin-Wang 单独执行；选定并验证具体源码提交，再发布资产，最后更新索引。

不要在 Aladdin-Wang 的个人仓库同时开发另一套主线。不得为了合并关闭规则集或必需 CI；本次取消审批门槛不授予其他规则变更权限。
